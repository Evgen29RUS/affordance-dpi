# trace_c0.py
# Identical to trace_b0.py with VARIANT = 'C'.
# Fresh 200k training of C, seed 0, per-rollout logging.
...
def delta_logit_w(logits_np, z_w):
    """Δlogit_w = (l1 + l3) − (l0 + l2)  — log-odds for a_w=1 vs a_w=0."""
    dl = (logits_np[:, 1] + logits_np[:, 3]
          - logits_np[:, 0] - logits_np[:, 2])
    d0 = dl[z_w == 0].mean() if (z_w == 0).sum() > 0 else 0.0
    d1 = dl[z_w == 1].mean() if (z_w == 1).sum() > 0 else 0.0
    return float(d1 - d0)
...
def evaluate_quick(agent, seed, n_ep=EVAL_N):
    ...
    # aux carries w in variant C
    aux_bacc_w = bacc((aux[:, 0] > aux[:, 1]).astype(int), z_w)
    ...
    return {
        ...
        'stoch_a_w_z_w':   bacc(a_stoch % 2, z_w),
        'stoch_a_u_z_u':   bacc(a_stoch // 2, z_u),
        'delta_logit_w':   delta_logit_w(logits_np, z_w),
        'entropy':         ent,
        'aux_to_z_w':      aux_bacc_w,
...
def main():
    print("=" * 78)
    print(f"TRACE {VARIANT}_{SEED} — {N_STEPS} steps (fresh training)")
    print("=" * 78)
    ...
    for it in range(n_iter):
        ...
        if (it + 1) % LOG_EVERY == 0 or it == 0 or it == n_iter - 1:
            ...
            print(f"  it {it+1:5d} ({row['steps']:>7d})  "
                  ...
                  f"a_w_arg={ev['arg_a_w_z_w']:.3f}  "
                  f"a_w_stoch={ev['stoch_a_w_z_w']:.3f}  "
                  f"a_u_arg={ev['arg_a_u_z_u']:.3f}  "
                  f"Δl_w={ev['delta_logit_w']:+.4f}  "
                  f"H={ev['entropy']:.3f}  "
                  f"aux→z_w={ev['aux_to_z_w']:.3f}  "
                  f"clip={step_log['clip_frac']:.3f}")

    with open('v5_3_trace_C0.json', 'w') as f:
        json.dump(history, f, indent=2)

    print("\nSaved to v5_3_trace_C0.json")
    ...
    if history:
        ...
        print("SUMMARY")
        print("=" * 78)
        print(f"  Δl_w:      {first['delta_logit_w']:+.4f} → {last['delta_logit_w']:+.4f}")
        print(f"  a_w_arg:   {first['arg_a_w_z_w']:.4f} → {last['arg_a_w_z_w']:.4f}")
        print(f"  arg_ret:   {first['arg_return']:+.4f} → {last['arg_return']:+.4f}")
        print(f"  stoch_ret: {first['stoch_return']:+.4f} → {last['stoch_return']:+.4f}")
        ...
        print(f"  a_u_arg (diagnostic, expect ~0.5): "
              f"{first['arg_a_u_z_u']:.4f} → {last['arg_a_u_z_u']:.4f}")

        # time to reach thresholds on argmax a_w
        print("\n  Time to threshold on a_w_arg (steps):")
        for thr in [0.70, 0.80, 0.90]:
            hit = None
            for r in history:
                if r['arg_a_w_z_w'] >= thr:
                    hit = r['steps']; break
            print(f"    ≥ {thr:.2f}: {hit if hit is not None else 'not reached'}")

        margins = np.array([r['delta_logit_w'] for r in history])
        n = len(margins)
        first_half = margins[:n//2].mean()
        second_half = margins[n//2:].mean()
        print(f"\n  mean Δl_w: first half={first_half:+.4f}  "
